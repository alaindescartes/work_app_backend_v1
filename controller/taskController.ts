...
export async function editTaskData(
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "Task edited successfully", task: editedTask });
  } catch (error: any) {
    if (process.env.NODE_ENV !== "production") {
      console.log("unable to edit task", error.message);
    }
    return next(
      new AppError(error.message || "an error occured while editing tasks", 500)
    );
    //TODO:log error to a logging system
  }
}

export async function getTaskBYHome(
  req: Request,
  res: Response,
  next: NextFunction
) {
  ...
  try {
    const fetchedTask: Task[] = await findTaskByGroupHome(
      Number(homeId),
      req.app.get("db")
    );
