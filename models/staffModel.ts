...
export type StaffInsert = Omit<StaffBase, 'staffId' | 'created_at' | 'updated_at'>;
//fetching staff
export type StaffFetch = Required<StaffBase>;

//function to add staff data
export async function addStaff(knex: Knex, staffData: StaffInsert): Promise<StaffFetch> {
...
export async function deleteStaff(knex: Knex, data: staffToDelete): Promise<staffToDelete> {
...
export async function getStaffData(knex: Knex, email: string): Promise<StaffFetch> {
...
export async function getStaffById(knex: Knex, id: number): Promise<StaffFetch> {
  const staff = await knex('staff').where({ staffId: id }).first();
  if (!staff) {
    throw new Error('Staff member not found.');
  }
  return staff;
